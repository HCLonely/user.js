...
(function () {
    ...
    userName = document.body.innerHTML.match(/https:\/\/steamcommunity\.com\/id\/([^\/]+)/)[1];

    try {
        steam64ID = JSON.parse($('#application_config').attr('data-userinfo')).steamid;
    } catch (e) {
        if (typeof UserConfig === 'object' && UserConfig?.steamid) {
            steam64ID = UserConfig.steamid;
        } else {
            console.error(e);
    ...
    (function start() {
        ...
        div.innerHTML = `
<div class="button_container" style="margin: 40px 40px 15px 40px;">
...
`;
        ...
        a.setAttribute("class", $('a[href^="https://help.steampowered.com/"]:first').attr("class"));
        a.setAttribute("style", "cursor:pointer");
        a.innerHTML = "一键移除";
        if ($(".supernav_container:first").length) {
            $(".supernav_container:first").append(a);
        } else {
            $('a[href^="https://help.steampowered.com/"]:first').next().after(a);
        }

        a.onclick = function () {
            if (window.location.host == "steamcommunity.com") {
                if (confirm("此功能需要在商店页面运行，是否跳转？")) window.open("https://store.steampowered.com/", "_self");
            } else if (!steam64ID && !userName) {
                    ...
                    if (data.status == 200) {
                        ...
                        if (followGame && (followGame.length > 0)) {
                            ...
                            for (let i = 0; i < gameList.length; i++) {
                                checkbox += `<p class="checkbox"><input type="checkbox" name="${gameList[i].name}" value="${gameList[i].id}"><a href="https://steamcommunity.com/app/${gameList[i].id}" target="_blank" style="color: #fff;">${gameList[i].name}</a></p>`;
                            }
                            $("#setting").html(checkbox);
                ...
                onload: function (data) {
                    ...
                    if (data.status == 200) {
                        ...
                        for (let i = 0; i < wishlistGame.length; i++) {
                            checkbox += `<p class="checkbox"><input type="checkbox" name="${wishlistGame[i]}" value="${wishlistGame[i]}"><a href="https://steamcommunity.com/app/${wishlistGame[i]}" target="_blank" style="color: #fff;">${wishlistGame[i]}</a></p>`;
                        }
                        $("#setting").html(checkbox);
                        p.innerHTML += '<font style="color:green">成功！</font>';
...
`);
        ...
        const htmlOld = document.getElementsByTagName("html")[0];
        setInterval(() => {
            const htmlNew = document.getElementsByTagName("html")[0];
            if ($('#remove_btn').length === 0 && htmlNew.getAttribute('class') !== htmlOld.getAttribute('class')) {
                start();
            }
        }, 1000);
    })();
