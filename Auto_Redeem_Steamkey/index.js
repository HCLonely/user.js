...
(function () {
      ...
      if ($('body').length > 0) {
        $('body').click((event) => {
          ...
          if ($(htmlEl).parents('.swal-overlay').length === 0 && !['A', 'BUTTON', 'TEXTAREA'].includes(htmlEl.tagName) && !['button', 'text'].includes(htmlEl.getAttribute('type'))) {
            if (($(htmlEl).children().length === 0 || !/([0-9,A-Z]{5}-){2,4}[0-9,A-Z]{5}/gim.test($.makeArray($(htmlEl).children()
              .map(function () {
                return $(this).text();
  ...
  function setUnusedKeys(key, success, reason, subId, subName) {
    if (success && allUnusedKeys.includes(key)) {
      var listObject;
      allUnusedKeys = allUnusedKeys.filter((keyItem) => keyItem !== key);
  ...
  function redeemKeys(key) {
    const keys = key || getKeysByRE($('#inputKey').val()
      .trim());
    redeem(keys);
  }
  ...
  function asfRedeem(command) {
    ...
    swal({
      ...
      if (/!redeem/gim.test(command)) {
        ...
        if (v) {
          const unUseKey = $('.swal-content textarea').val()
            .split(/[(\r\n)\r\n]+/)
            .map((e) => {
              if (/未使用/gim.test(e)) {
                return e;
