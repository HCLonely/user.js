...
// @require     https://greasyfork.org/scripts/389177-static/code/static.js?version=786747
...
<input type="checkbox" name="asf" ${setting.asf ? 'checked=checked' : ''} title="此功能默认关闭新标签页激活"/><span title="此功能默认关闭新标签页激活">开启ASF激活</span>
        ...
        swal({
          ...
          .then((value) => {
            if (value) {
              const setting = {}
              const allSetting = $jQuery('#hclonely-asf input').map(function () {
          ...
          GM_xmlhttpRequest({
            ...
            onload: function (data) {
              if (data.finalUrl.includes('login')) {
                ...
                if (data.status == 200) {
                  const session_id = data.responseText.match(/g_sessionID = \"(.+?)\";/)
      ...
      function redeemSub (e) {
        ...
        if (subText) {
          ...
          let modal = swal('正在执行…', '请等待所有请求完成。 忽略所有错误，让它完成。')
          for (; i < total; i++) {
